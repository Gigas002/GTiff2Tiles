...
RUN ls -a
RUN dotnet restore GTiff2Tiles.Core.csproj
RUN dotnet restore GTiff2Tiles.Console.csproj
RUN ls -a

# copy and publish app and libraries
COPY . ./
RUN dotnet publish GTiff2Tiles.Core.csproj -c release -o /app --no-restore /p:Platform=x64
RUN dotnet publish GTiff2Tiles.Console.csproj -c release -o /app --no-restore /p:Platform=x64
RUN ls -a
