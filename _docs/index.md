...
Preferred way to build the solution is to use **VS2019 (16.8.5+)**, but you can also build projects, using dotnet CLI in your terminal or in **VSCode (1.53.2+)** with **ms-dotnettools.csharp (1.23.9+)** extension.
Projects targets **.NET 5.0.3**, so you’ll need **.NET 5.0.103 SDK**.
